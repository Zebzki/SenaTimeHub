...
namespace Sena_TimeHub.vista
{
    public partial class recuperarContrasena : System.Web.UI.Page
    {
        ...
        protected void btnEnviarCorreo_Click1(object sender, EventArgs e)
        {
            ...
            clAprendizE objDatosA = objRC.mtdRecuperarContrasenaAp(correo, 0, null);
            ...
            else if (objDatosA.validarAprendiz == true)
            {
                string nombreCompleto = objDatosA.nombreAprendiz + " " + objDatosA.apellidoAprendiz;
                Session["idAprendiz"] = objDatosA.idAprendiz.ToString();
                ...
                bool validacion = objEnviarCodigo.enviarCorreo(objDatosA.emailAprendiz, nombreCompleto, asunto, cuerpo);
                if (validacion == true)
                {
                    ...
                    txtParametros.TextMode = TextBoxMode.SingleLine;
                    btnEnviarCorreo.Visible = false;
                    txtMensajePrincipal.InnerHtml = "Ingresa el codigo de cuatro dijitos enviado a (<b>" + objDatosA.emailAprendiz + "</b>).";
                    txtMensaje2.Visible = false;
                    btnVerificarCodigo.Visible = true;
        ...
        protected void btnActualizarC_ServerClick(object sender, EventArgs e)
        {
            ...
            if (contrasenaIngresada.Length >= 8)
            {
                ...
                else
                {
                    ...
                    }else if (Session["idAprendiz"] != null)
                    {
                        int idAprendiz = int.Parse(Session["idAprendiz"].ToString());

                        clAprendizE objAprendizValidacion = objRP.mtdRecuperarContrasenaAp(null, idAprendiz, txtContrasena.Text);

                        if (objAprendizValidacion.validarAprendiz == true)
                        {
                            Session.Clear();
                            Session.Abandon();
            ...
            else
            {
                ...
                txtMensaje3.Attributes["style"] = "color: darkred;";
                txtMensaje3.InnerText = "La contraseña debe tener al menos 8 caracteres.";
